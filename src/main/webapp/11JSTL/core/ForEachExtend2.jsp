...
<body>
	...
	items에 지정한 컬렉션에 저장된 객체의 개수만큼 반복하여 list로 하나씩
	전달한다. -->
	...
	<h4>Java코드를 통한 출력</h4>
	<%
	/* Java코드를 사용하면 출력할 객체를 명시하고, getter()메소드를 직접
	호출해야한다.*/
	for(Person p:lists){
	...
	for(String key:keys){
		...
		/* Key와 Value를 출력한다.
		여기서 Value는 Person객체이므로 멤버변수를 통해 값을 출력할 수 있다.*/
		out.println("Key=>"+key+"<br>");
		out.println("Value=>"+p.getName()+", "+p.getAge()+"<br>");
	}
	/* JSTL의 확장 for문을 사용하면 Key를 별도로 얻어와야하는 번거로움이
	없으므로 편리하다.*/
	%>
</body>
</html>